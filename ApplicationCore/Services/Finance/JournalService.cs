...
//        // V2 Code
//        public async Task<List<JournalEntryDto>> GetJournalEntriesAsync(EntryQuery query)
//        {
//            var result = _transactionMasterRepository.ListQueryable(new JournalEntrySpecification(query)).Include(t => t.TransactionDetails).ThenInclude(d => d.Account) .ProjectTo<JournalEntryDto>().ToList();
